$(document).ready(function(){
    ...
    function progressBarVertical(secs, secsIncrement) {
        for (let i=secs; i>=0; i-=secsIncrement) {
            setTimeout(()=>{
                $(".play__v-progressBar-bar").css("flex","0 0 "+(i*100/secs)+"%");
            },(secs*1000-1000*i))
        }
    }
    ...
    // </div>
        ...
        submit_button_group.className = "decide__submit--button decide__answers-submit";
            let submit_button = document.createElement("button");
            submit_button.className = "decide__submit-decision login__button login__button--orange";
    ...
    function displayDecideData_user_block(data) {
        ...
        user_placeholder.className = "decide__rail-group decide__rail-group--category";
    ...
    function displayDecideData_user_resp_block(data,categories) {
        ...
        categories.forEach((category,categoryIndex)=>{
            ...
            response_rail_group.className = "decide__rail-group decide__rail-group--category";
            ...
            data.forEach((user,userIndex)=>{
                ...
                if (response) {
                    ...
                    options.forEach((option)=>{
                        ...
                        response_checkbox.className = "decide__radio--res decide__radio--"+option;
                        response_checkbox.setAttribute("name",userIndex+"-"+categoryIndex); // element.name is readOnly
                    ...
                    let response_checkbox = document.createElement("input");
                    response_checkbox.type = "radio";
                    response_checkbox.className = "decide__radio--res decide__radio--incorrect";
                    response_checkbox.setAttribute("name",userIndex+"-"+categoryIndex); // element.name is readOnly
    ...
    $('.login__button-create').on('click', function() {
        generateRoomCode(function(roomCode) {
            window.location = "/#create";
            $('#room_create').attr('placeholder',`Room Code: ${roomCode}`);
            // Store the roomCode in the rel attribute for verification purposes
            $('#room_create').attr('rel',roomCode);
        })
    })
    ...
    $('.login__form-create').on('submit',function(e) {
        ...
        var roomCode = $('#room_create').attr('rel');
        if(isdefined(typeof(roomCode))) {
            ...
            socket.emit("login--create-room",{userName, roomCode, role: "admin"},function(error,message) {
                console.log(message);
                if (!error) {
                    // Join Socket To Room
    ...
    $('.login__form-join').on('submit',function(e) {
        ...
        socket.emit("login--room-exists",{roomCode},function(errorA,message) {
            if (!errorA) {
                ...
                socket.emit("login--join-room",{userName, roomCode, role: "user"},function(errorB, message) {
                    if (!errorB) {
                        window.location = "/#";
                        triggerLobby();
                        socket.emit("lobby--joined");
    ...
    socket.on("lobby--build", function(scoreboard,playerLimit){
        ...
        isAdmin(function(isAdmin) {
            ...
            scoreboard.forEach((player)=>{
                let user_rail = $(`<div class='lobby__rail lobby__rail--${player.pos}'></div>`);
                user_rail.append($(`<span class='lobby__rail--name'>${player.name}</span>`));
                user_rail.append($(`<span class='lobby__rail--score'>${player.points}</span>`));
                $(".lobby__ladder--js").append(user_rail);
            })
            $(".lobby__header--sub").text("Players: "+scoreboard.length+"/"+playerLimit)
        })
    ...
    socket.on("decide--build",function(responses,categories){
        ...
        isAdmin(function(isAdmin){
            ...
            document.querySelector(".decide__answers-container").appendChild(flexData);
            ...
            $(".decide__input").on("click",function(){
                ...
                $(this).parent().find(`input[value='${decide_option}']`).prop("checked",true)
            })
        })
