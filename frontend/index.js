...
app.get("/get-data", function (req, res, next) {
  ...
  mongo.connect(url, function (err, client) {
    ...
    var cursor = db
      ...
      .find({ name: { $ne: "Lasith" } })
      .limit(1); // Connect to gameStates collection
    ...
    socket.join(roomCode, function () {
      ...
      admin__mongo_createRoom(
        ...
        function () {
          acknowledge(
            null,
            "Successly Joined Room And Created Entry in Database: " + roomCode
          );
        }
      );
    });
          ...
          function (err, result) {
            ...
            if (util.isdefined(typeof newAdminSocketId)) {
              db.collection("gameStates").update(
                { _id: roomCode },
                {
                  $set: { ["players." + newAdminSocketId + ".role"]: "admin" },
                },
                function (err, result) {
                  assert.equal(null, err);
                  ...
                  client.close();
                  callback(
                    "Admin has left and a new user has been allocated admin"
                  );
                }
              );
            } else {
              db.collection("gameStates").deleteOne(
                { _id: roomCode },
                function (err, result) {
                  client.close();
                  callback(
                    "All players have left and the room has been removed from the db"
                  );
                }
              );
            }
...
function admin__mongo_updateRules(roomCode, rules, callback) {
  ...
  mongo.connect(url, function (err, client) {
    ...
    db.collection("gameStates").updateOne(
      ...
      {
        $set: {
          "gameData.roundsRemaining": db_rounds,
          "gameData.roundTime": db_round_time,
        },
      },
      function (err, result) {
        assert.equal(null, err);
...
function user_mongo_joinRoom(roomCode, userData, callback) {
  ...
  mongo.connect(url, function (err, client) {
    ...
    db.collection("gameStates").update(
      { _id: roomCode },
      { $set: { ["players." + userSocketID]: subDoc } },
      function (err, result) {
        assert.equal(null, err);
        //console.log(result);
        client.close(); // Close connection
      ...
      function (err, result) {
        ...
        if (result) {
          var playersPointsArray = Object.keys(result.players).map((key) => {
            return {
              name: result.players[key].name,
              points: result.players[key].points,
              pos: result.players[key].pos,
            };
          });
...
function mongo__roundData(roomCode, callback) {
    ...
    mongo.connect(url, function (err, client) {
      ...
      db.collection("gameStates").findOne(
        ...
        function (err, result) {
          ...
          db.collection("gameStates").updateOne(
            { _id: roomCode },
            { $set: { "gameData.availableLetters": availableLetters } },
            function (err, result) {
              assert.equal(null, err);
              client.close();
  ...
  Promise.all([fetchCategoriesAndInsert, fetchRoundData]).then(function (
    values
  ) {
...
function mongo__insertResponse(socketID, userName, roomCode, response) {
  return new Promise((resolve, reject) => {
    mongo.connect(url, function (err, client) {
      ...
      db.collection("gameStates").findOneAndUpdate(
        { _id: roomCode },
        { $push: { responses: { socketID, userName, response } } },
        ...
        function (err, doc) {
          ...
          resolve(
            Object.keys(doc.value.players).length === doc.value.responses.length
          );
        }
      );
    });
...
function mongo_deincrementRoundCount(roomCode) {
  mongo.connect(url, function (err, client) {
    ...
    db.collection("gameStates").updateOne(
      { _id: roomCode },
      { $inc: { "gameData.roundsRemaining": -1 } },
      function (err, result) {
        assert.equal(null, err);
        client.close();
      }
